...
const Contributors: FC<ContributorsComponentProps> = ({ contributors }) => {
  const [showMorePositions, setShowMorePositions] = useState<string[]>([]);
  ...
  const [filterformValues, setFilterformValues] = useState<FormValues>({
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newFilteredContributors = filteredContributors.filter((contributor) =>
      JSON.stringify(contributor).toLowerCase().includes(term),
    );
  ...
  const getAffiliationsArray = () => {
    const affiliationSet: Set<string> = new Set();
    contributors.forEach((contributor) => {
      const affiliation = contributor?.affiliation;
      affiliationSet.add(affiliation);
    });
    const affiliationArray: string[] = [...affiliationSet];
  ...
  const renderMore = (id: string) => {
    setShowMorePositions([...showMorePositions, id]);
  };

  const renderPositions = (contributor: Contributor) => {
    if (!contributor?.positions) return null;
          ...
          contributor.positions.slice(3).map((position, index) => {
            return (
              <div className="text-gray-500" key={position}>
                {position}
                {index ===
                (contributor?.positions as string[]).slice(3).length - 1 ? (
                  <span
                    className="pl-2 text-elixirblue cursor-pointer"
                    onClick={() =>
  ...
  const isActiveContributor = (contributor: any) => {
  ...
  const isProjectLead = (contributor: Contributor) => {
  ...
  const hasRole = (role: string, contributor: Contributor) => {
    const position = contributor.positions;
  ...
  const filterContributors = useCallback(() => {
    ...
    const roles = filterformValues.rolesInput;
    const affiliations = filterformValues.affiliationsInput;

    let filterContributors = contributors;
    if (!includePastContributor)
      filterContributors = filterContributors.filter((contributor) =>
        isActiveContributor(contributor),
      );

    if (projectLeadsOnly)
      filterContributors = filterContributors.filter((contributor) =>
        isProjectLead(contributor),
      );

    if (roles.length > 0) {
      filterContributors = filterContributors.filter((contributor) => {
        return roles.some((role) => hasRole(role, contributor));
      });
    }
    ...
    setFilteredContributors([...filterContributors]);
  }, [filterformValues]);
  ...
  const renderLinks = (contributor: Contributor) => {
    return (
      <div className="flex items-baseline pt-0.5 space-x-2">
        ...
        {contributor.orcid ? (
          <a href={contributor.orcid}>
            <svg
              fill={context?.theme === 'light' ? '#000000' : '#ffffff'}
              height="24px"
              viewBox="0 0 24 24"
              width="24px"
